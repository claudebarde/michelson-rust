...
pub fn run(
    ...
    // checks the stack
    match stack.check_depth(options.pos + 1, Instruction::GT) {
        Ok(_) => (),
        Err(err) => panic!("{}", err),
    };
...
mod tests {
    ...
    fn eq_success() {
        ...
        match run(initial_stack, &options, stack_snapshots) {
            ...
            Ok((stack, _)) => {
                ...
                assert_eq!(stack[0].instruction, Instruction::GT);
    ...
    #[test]
    #[should_panic(
        expected = "Unexpected stack length, expected a length of 1 for instruction GT, got 0"
    )]
    fn eq_empty_stack() {
        let initial_stack: Stack = vec![];
        let stack_snapshots = vec![];
    ...
    #[test]
    #[should_panic(expected = "Invalid type for `GT` expected int, but got nat")]
    fn eq_invalid_type() {
        let initial_stack: Stack = vec![
