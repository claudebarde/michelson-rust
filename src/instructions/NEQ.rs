...
pub fn run(
    ...
    // checks the stack
    match stack.check_depth(options.pos + 1, Instruction::NEQ) {
        Ok(_) => (),
        Err(err) => panic!("{}", err),
    };
    ...
    match stack[options.pos].value {
        MValue::Int(val) => {
            ...
            let new_stack = stack.replace(
                vec![StackElement::new(new_val, Instruction::NEQ)],
                options.pos,
            );
...
mod tests {
    ...
    fn eq_success() {
        ...
        match run(initial_stack, &options, stack_snapshots) {
            ...
            Ok((stack, _)) => {
                ...
                assert_eq!(stack[0].instruction, Instruction::NEQ);
    ...
    #[test]
    #[should_panic(
        expected = "Unexpected stack length, expected a length of 1 for instruction NEQ, got 0"
    )]
    fn eq_empty_stack() {
        let initial_stack: Stack = vec![];
        let stack_snapshots = vec![];
    ...
    #[test]
    #[should_panic(expected = "Invalid type for `NEQ` expected int, but got nat")]
    fn eq_invalid_type() {
        let initial_stack: Stack = vec![
